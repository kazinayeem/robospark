...
// NEW TSHIRT DROPDOWN
// -----------------
const TshirtSelect = ({ label, value, onChange }: any) => (
  <div className="space-y-1 w-full">
    <label className="text-cyan-300 text-sm font-semibold">{label}</label>
    <select
      value={value}
...
const App: React.FC = () => {
  ...
  const competitionFees: any = {
    ...
    "Cozmo Clench": { base: 1500, extra: 500 },
  };
  ...
  // -----------------
  // UPDATED FORM DATA (T-SHIRT INCLUDED)
  // -----------------
  ...
  // -----------------
  // UPDATED FOR T-SHIRT FIELD
  // -----------------
  const handleMemberChange = (
    index: number,
    field: "name" | "email" | "phone" | "tshirt",
    value: string
  ) => {
  ...
  const validate = () => {
    return (
      ...
      formData.leaderTshirt && // NEW REQUIRED FIELD
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        title: "Success!",
        html: `
          Your team has been registered!<br/><br/>
          📧 Please check your email for the guide.<br/>
          ⚠️ Also check <b>Inbox</b>, <b>Spam</b>, and <b>Promotions</b> folders.
        `,
      });
  ...
  // -----------------
  // LOADING & CLOSED HANDLING
  // -----------------
  if (regOpen === null) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-cyan-300 text-xl">
  ...
  if (regOpen === false) {
    return (
      ...
      bg-gradient-to-br from-gray-950 via-indigo-900 to-black px-4 py-20 pt-32"
      >
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-['Orbitron'] font-extrabold text-red-400">
          ❌ Registration Closed
        </h1>

        <p className="text-gray-300 mt-4">
          Thank you for your interest in{" "}
          <span className="text-cyan-400 font-bold">RoboSpark 2025</span>.
        </p>
        ...
        <button
          ...
          className="mt-8 px-6 py-3 bg-cyan-600 text-black rounded-xl"
        >
          Refresh
        </button>
  ...
  return (
    <div className="min-h-screen pt-32 bg-gradient-to-br from-gray-950 via-indigo-900 to-black text-white p-6">
      <div className="text-center">
        <h1 className="text-3xl md:text-5xl font-['Orbitron'] font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600">
          ROBOSPARK 2025
        </h1>
      ...
      >
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {/* NEW LEADER TSHIRT FIELD */}
            <TshirtSelect
              label="Leader T-Shirt Size"
              value={formData.leaderTshirt}
              onChange={(v: string) =>
                setFormData({ ...formData, leaderTshirt: v })
              }
            />
          </div>
        </section>
          ...
          {formData.members.map((m, index) => (
            ...
            >
              ...
              <div className="grid md:grid-cols-4 gap-4">
                ...
                {/* NEW MEMBER TSHIRT FIELD */}
                <TshirtSelect
                  label="T-Shirt Size"
                  value={m.tshirt}
                  onChange={(v: string) =>
                    handleMemberChange(index, "tshirt", v)
                  }
                />
              </div>
            </div>
            ...
            <div className="bg-gray-800/40 p-4 rounded-lg border border-cyan-700/30 mt-4">
              ...
              {formData.paymentMethod === "bkash" && (
                <p className="text-pink-400 text-lg font-bold">
                  bKash: 01553081243 (Send Money)
                </p>
              )}

              {formData.paymentMethod === "rocket" && (
                <p className="text-yellow-400 text-lg font-bold">
                  Rocket: 015530812437 (Send Money)
                </p>
              )}
            </div>
          )}
