...
export default function TeamsPage(): JSX.Element {
  ...
  const fetchTeams = async () => {
    ...
    try {
      ...
      snap.forEach((d: QueryDocumentSnapshot<DocumentData>) => {
        // ensure we always have members array structure and default fields
        ...
        const normalized: Team = {
          ...
          senderNumber: raw.senderNumber ?? "",
          transactionId: raw.transactionId ?? "",
          couponCode: raw.couponCode ?? "",
          fees: raw.fees ?? null,
          paymentAccepted: !!raw.paymentAccepted,
  ...
  const exportPDF = () => {
    try {
      ...
      Swal.fire("Error", "Failed to generate PDF. Check console.", "error");
    }
  };
  ...
  const exportExcel = () => {
    try {
      ...
      const filename = `robospark_teams_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;
  ...
  const addEditingMember = () => {
    setEditingTeam((prev) => {
      ...
      members.push({ name: "", email: "", phone: "", tshirt: "" });
      return { ...prev, members };
    });
  };
  ...
  const saveEdit = async () => {
    ...
    setSavingEdit(true);
    try {
      const cleanedMembers = (editingTeam.members ?? [])
        .map((m) => ({
          name: m.name ?? "",
          email: m.email ?? "",
          phone: m.phone ?? "",
      ...
      const payload: any = {
        ...
        leaderTshirt: editingTeam.leaderTshirt ?? "",
        ...
        couponCode: editingTeam.couponCode ?? "",
      ...
      if (newStatus === true) {
        try {
          await fetch(`${API_URL}/api/payment-mail`, {
            ...
            // send updated team object (with paymentAccepted true)
            body: JSON.stringify({ ...team, paymentAccepted: true }),
          });
  ...
  const exportLeaderEmails = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Leader Emails");

      const filename = `robospark_leader_emails_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;

      XLSX.writeFile(workbook, filename);
    } catch (err) {
      console.error("exportLeaderEmails error:", err);
      Swal.fire("Error", "Unable to export leader emails.", "error");
    }
  };
  const exportAllEmails = () => {
    try {
      ...
      const filename = `robospark_all_emails_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;

      XLSX.writeFile(workbook, filename);
    } catch (err) {
      console.error("exportAllEmails error:", err);
      Swal.fire("Error", "Unable to export all emails.", "error");
    }
  };
  ...
  return (
    <div className="min-h-screen pt-28 px-4 pb-12 bg-gradient-to-br from-black via-gray-900 to-blue-950 text-white">
      ...
      <div className="max-w-6xl mx-auto mb-8 space-y-4">
        {/* Row 1: Search + Filter + Reset + Refresh */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <button
            ...
            className="px-4 py-3 bg-cyan-600 rounded-xl text-black font-semibold w-full"
          >
            Reset
          </button>
          ...
          <button
            onClick={fetchTeams}
            className="px-4 py-3 bg-indigo-700 rounded-xl hover:bg-indigo-600 w-full"
          >
            Refresh
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <button
            onClick={exportPDF}
            className="px-4 py-3 bg-red-600 rounded-xl hover:bg-red-500 font-semibold w-full"
          >
           Export PDF
          </button>
          ...
          <button
            onClick={exportExcel}
            className="px-4 py-3 bg-green-600 rounded-xl hover:bg-green-500 font-semibold w-full"
          >
           Export Excel
          </button>

          <button
            onClick={exportLeaderEmails}
            className="px-4 py-3 bg-orange-600 rounded-xl hover:bg-orange-500 font-semibold w-full"
          >
           Export Leader Emails
          </button>

          <button
            onClick={exportAllEmails}
            className="px-4 py-3 bg-purple-600 rounded-xl hover:bg-purple-500 font-semibold w-full"
          >
          Export  All Emails
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto mt-4 mb-6 p-4 bg-gray-800/60 rounded-xl border border-cyan-600/30">
        {/* TOTAL REGISTRATIONS — SMALL UI */}
        ...
        {/* COMPETITION WISE — SMALL UI */}
        <p className="text-sm text-cyan-300 font-semibold mb-2">
          Competition-wise Registrations
        </p>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {Object.entries(topicCounts).map(([comp, count]) => (
            <div
              key={comp}
              className="p-2 bg-gray-900/70 rounded-md border border-cyan-600/20 text-center"
            >
              <p className="text-xs text-cyan-400 font-medium">{comp}</p>
              <p className="text-xl font-bold text-cyan-300">{count}</p>
            </div>
          ))}
        </div>
      </div>
              ...
              <div className="mt-3">
                ...
                {(team.members ?? []).map((m, i) => (
                  <div key={i} className="mt-2 bg-gray-800/40 p-2 rounded-md">
                    ...
                    <p className="text-xs">👕 T-Shirt: {m.tshirt || "—"}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={editingTeam.leaderTshirt ?? ""}
                    onChange={(e) =>
                      updateEditingField("leaderTshirt", e.target.value)
                    }
                    className="w-full px-3 py-2 rounded-md bg-gray-800 border border-gray-700"
                  >
                      ...
                      <div className="flex justify-between items-start gap-3">
                        <div className="flex-1 grid md:grid-cols-4 gap-2">
                          ...
                          <select
                            value={m?.tshirt ?? ""}
                            onChange={(e) =>
                              updateEditingMember(idx, "tshirt", e.target.value)
                            }
                            className="px-3 py-2 rounded-md bg-gray-900 border border-gray-700"
                          >
